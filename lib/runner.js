...
const PROGRESS_BAR = 'reindexing [:bar] :current/:total (:percent) :elapsed'
  ...
  fetchScroll (scrollId, retries) {
    ...
    return this.from.client.scroll({
      ...
      .catch((err) => {
        ...
        if (retries) {
          this.logger.info(`Retrying scroll (${retries - 1} attempts remaining)`)

          return wait(this.options.request_retry_wait).then(() => {
            this.fetchScroll(scrollId, retries - 1)
          })
        } else {
      ...
      .then(() => {
        return this.from.client.clearScroll({ scrollId }).catch(() => {});
      })
  }
  ...
  pushBulk (docs, retries) {
    ...
    return this.to.client.bulk({ body: queue })
      .catch((err) => {
        ...
        if (retries) {
          this.logger.info(`Retrying bulk (${retries - 1} attempts remaining)`)

          return wait(this.options.request_retry_wait).then(() => {
            this.pushBulk(docs, retries - 1)
          })
        } else {
